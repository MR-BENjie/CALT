...
class PolicySequencingTrainer(Trainer):
        ...
        while ps_epoch < config.ps_epochs:
            ...
            for i in range(num_agent):
                ...
                runner = self._runner.run(
                    every_steps=1,every_episodes= 1, log_prefix=f"subtask {i}"
                )
                done = False
                ...
                while not done:
                    ...
                    # train an agent
                    _train_info = self._agent.train(i, rollout)
    ...
    def evaluate(self):
        ...
        self.info = logger.info("Run %d evaluations at step=%d, ps_epoch=%d, update_iter=%d", self._config.num_eval,
                                step, ps_epoch, update_iter, )
