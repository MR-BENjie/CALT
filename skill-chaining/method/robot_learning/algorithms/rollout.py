...
class RolloutRunner(object):
    ...
    def run(
        ...
        while True:
            ...
            while not done:
                ...
                if self.task_flag == 'nutbolt':
                    ac = agent.get_action(ob, agent.is_determenistic)
                ...
                if config.absorbing_state and done_mask == 0 and self.task_flag != 'nutbolt':
    ...
    def run_episode(self, is_train=True, record_video=False):
        ...
        if il:
            ep_rew_il = 0
        if self.task_flag == 'nutbolt':
            ob_next = agent.env_reset(agent.env)
        else:
            ob_next = env.reset()
        ...
        while not done:
            ...
            # sample action from policy
            if self.task_flag == 'nutbolt':
                obs = agent.obs_to_torch(ob)
                #print(obs)
                if has_masks:
                    masks = env.get_action_mask()
                    ac = agent.get_masked_action(
                        torch.unsqueeze(obs,dim=0), masks, agent.is_determenistic)
                else:
                    ac = agent.get_action(torch.unsqueeze(obs,dim=0),agent.is_determenistic)
                if not agent.is_tensor_obses:
                    ac = ac.cpu().numpy()
                ac_before_activation = ac
                #print(ac[5])
            else:
            ...
            # take a step
            if self.task_flag == 'nutbolt':
                ob_next, reward, done, info = agent.env_step(agent.env,torch.unsqueeze(ac,dim=0))
                if env._env_name == 'Screw':
                    #print(env._get_curr_failures(env._get_curr_successes()))
                    if env._get_curr_failures(env._get_curr_successes()):
                        done = True
                        info['successes'] = False
                    if env._get_curr_successes():
                        done = True
                        env.reset_buf = torch.ones_like(env.reset_buf)
                        info['successes'] = True
                if hasattr(ob_next, 'dtype') and ob_next.dtype == np.float64:
                    ob_next = np.float32(ob_next)
                if agent.render_env:
                    agent.env.render(mode='human')
                    time.sleep(agent.render_sleep)
            else:
                ob_next, reward, done, info = env.step(ac)
            if il:
                reward_il = agent.predict_reward(ob, ob_next, ac)
            ...
            #print(reward_info['successes'])
            if il:
                ep_info["rew_il"] = ep_rew_il
            if self.task_flag == 'nutbolt':
                ep_info["episode_success"] = reward_info['successes']
                if reward_info['successes']:
                    ep_info["episode_success_state"] = ob_init
            else:
