...
def load_player(path):
    ...
    if 'Pick' in path:
        task_config = 'config_Pick.yaml'
    elif 'Place' in path:
        task_config = 'config_Place.yaml'
    elif 'Screw' in path:
        task_config = 'config_Screw.yaml'
    else:
        assert False
    ...
    task = (task_config.split(".")[0]).split("_")[1]
    task = "FactoryTaskNutBolt" + task

    def load_player_(cfg, env_info):
        args = {
            'train': not cfg.test,
            'play': cfg.test,
        ...
        return player
    player = load_player_(cfg, env_info)
    player.env = env
    return player, env
class Trainer(object):
    ...
    def __init__(self, config):
        ...
        if self._config.init_ckpt_path in ['Pick','Place','Screw']:
            self._agent, self._env = load_player(self._config.init_ckpt_path)
            self._env._env_name = self._config.init_ckpt_path
            ob_space = ''
            ac_space = ''
        elif self._config.ps_ckpts[0] == 'PPS':
            ob_space = gym.spaces.Dict(spaces=dict())
            ob_space.spaces["absorbing_state"] = gym.spaces.Box(-float('inf'),float('inf'),[32])
            ac_space = Box(-1,1,[12])
            env_ob_space = Box(-float('inf'),float('inf'),[32])
        else:
        ...
        if self._config.init_ckpt_path in ['Pick', 'Place', 'Screw']:
            ...
            config, ob_space, ac_space, env_ob_space,task = 'PPS'
            )
    ...
    def _save_ckpt(self, ckpt_num, info=None):
        ...
        """
        if self._config.ps_ckpts[0] == 'PPS':
            tasks = self._agent.tasks
            for i in range(3):
                fn = os.path.join(self._config.log_dir, tasks[i]+"ckpt_%011d.pt" % ckpt_num)
                #env = self._agent[i].vec_env
                #self._agent[i].vec_env = None
                state = self._agent[i].get_full_state_weights()
                torch_ext.save_checkpoint(fn, state)
                #self._agent[i].vec_env = env
        else:
    ...
    def _load_ckpt(self, ckpt_path, ckpt_num):
        ...
        """
        if 'Pick' in ckpt_path or 'Place' in ckpt_path or 'Screw' in ckpt_path:

            self._runner.task_flag = 'nutbolt'
            return {}
        else:
            self._runner.task_flag = 'furniture'
            config = self._config
    ...
    def _evaluate(self, step=None, record_video=False):
        ...
        rollouts = []
        info_history = Info()
        if self._runner.task_flag == 'nutbolt':
            if self._runner._agent.is_rnn:
                self._runner._agent.init_rnn()
